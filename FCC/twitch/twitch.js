$(document).ready(function() {
  json.forEach(function(item) {
    var icon, iconItem, name, status, link, cls;
    if (item.stream == null) {
      // Offline
      cls = 'offline';
      name = item.display_name;
      status = 'Offline';
      iconItem = $('<td><i class="fa fa-question fa-2x"></i></td>')
      link = 'https://www.twitch.tv/' + name;
    } else {
      cls = 'online'
      name = item.stream.display_name;
      status = item.stream.status;
      icon = item.stream.logo;
      iconItem = $('<td></td>')
        .append($('<img class="logo"/>').attr('src', icon));
      link = item.stream.url;
    }
    if (name != undefined) {
      var newTags = $('<tr class="' + cls + '"></tr>')
        .append(iconItem)
        .append($('<td></td>')
                .append($('<a/>')
                        .attr('href', link)
